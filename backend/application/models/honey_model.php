...
class honey_model extends CI_Model
{
   ...
   * sin( radians(latitude))))<=10");
     ...
     * @return boolean $result : state of subtract
     */
    function catchBackyardHoney($no, $amount)
    {
        $this->db->select("amount");
        ...
        if($result[0]->amount == 1*$amount)
        {
            ...
            $this->db->query("update honey_backyard set amount=(amount-".(1*$amount).") where no=$no");
        }
        return true;
    }
    ...
    function addBackyard($user_id, $amount)
    {
        ...
        $limit = $this->db->query("select value from rule where no=6;")->result();
        $tempx=0;
        $tempy=0;
        if(count($temp) > 0){
            $sum = 0;
